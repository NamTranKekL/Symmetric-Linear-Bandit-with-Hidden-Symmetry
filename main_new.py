...
import itertools
from Partitions import Refine_Partition_NC, projection_matrix_partition, matrix_Sparsity_IntervalPartition
...
with tqdm(total=1) as pbar:
 ...
 #           print(n)
            ...
            ##### Model selection
            for i in range(2,d0+1):
            #    print(i)
                CollectionNeighbor = Refine_Partition_NC(partition_best)
 ...
 #           error_LS = np.linalg.norm(theta_est - theta)
  ...
  #          print("error LS:",error_LS)
            ...
            phi_Lasso = copy.deepcopy(Lasso(alpha=0.01).fit(Z, Y).coef_)  # alpha is the regularised constant, if s large, alpha should be large.
...
#            print("Lasso Risk Error = ", error_Lasso)
       ...
       #     Data_save.append(regret_data)
        ...
        excel_file_path = 'data_greedy' + '.xlsx'
