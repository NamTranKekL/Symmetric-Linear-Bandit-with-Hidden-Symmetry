...
import itertools
from Partitions import projection_matrix_partition, matrix_Sparsity_IntervalPartition, generate_list_of_lists, check_partition, Coarsen_Partition_Interval, random_interval_partition, Random_Sparse_Vector
...
d = 80
d0 = 10
Iter_T = np.linspace(500,20000,100)
SampleEachT = 100
...
#            phi_Lasso = copy.deepcopy(Lasso(alpha=4 * 0.1*np.sqrt(np.log(d)/ n)).fit(Z, Y).coef_)  # alpha is the regularised constant, if s large, alpha should be large.
 ...
 #           print("Lasso Risk Error = ", error_Lasso)
    ...
    excel_file_path = 'data_Lasso_Sparsity_'+str(d) + '.xlsx'
