...
import itertools
from Partitions import projection_matrix_partition, matrix_Sparsity_IntervalPartition, generate_list_of_lists, check_partition,random_partition, Coarsen_Partition_Interval, random_interval_partition, Random_Sparse_Vector
...
with tqdm(total=1) as pbar:
  ...
  #          print(insample)
           # partition_true = random_interval_partition(d,d0)
           # theta_raw = 10 * np.random.randn(d)
            #theta = projection_matrix_partition(partition_true, d) @ theta_raw
...
#            X = np.sqrt(d)*normalize(np.random.randn(n, d), axis=1, norm='l2')
            ...
            ##### Model selection
            W_ip_to_sparsity = matrix_Sparsity_IntervalPartition(d) # from interval partition to sparsity
  #          print(W_sparse_to_ip)
            Z = X @ W_ip_to_sparsity
            phi_est = np.linalg.inv(Z.transpose() @ Z) @ Z.transpose() @ Y
            theta_est = W_ip_to_sparsity @ phi_est
            ##################################################################################################################
            partition = generate_list_of_lists(d)
            partition_best = partition

            Proj = projection_matrix_partition(partition, d)
            PredictionError = np.linalg.norm(Y - Z @ Proj @ phi_est)
            PredictionError_best = 10000000

            for i in range(1,d-d0):
                PredictionError_best = 10000000
#                print(i)
                CollectionNeighbor = Coarsen_Partition_Interval(partition_best,d)
            ...
            phi_ms = projection_matrix_partition(partition_best,d) @ phi_est
            theta_ms =  W_ip_to_sparsity @ phi_ms
...
#            phi_Lasso = copy.deepcopy(Lasso(alpha=4 * 0.1*np.sqrt(np.log(d)/ n)).fit(Z, Y).coef_)  # alpha is the regularised constant, if s large, alpha should be large.
            beta = cp.Variable(d)
            lambd_b = 4 * 0.1 * np.sqrt(np.log(d) * n)
            lassosol = cp.Problem(cp.Minimize(objective_fn(X, Y, beta, lambd_b)))
            lassosol.solve()
            theta_Lasso = beta.value
 ...
 #           print("Lasso Risk Error = ", error_Lasso)
    ...
    excel_file_path = 'data_greedy_Sparsity_'+str(d) + '.xlsx'
