...
import itertools
from Partitions import projection_matrix_partition, matrix_Sparsity_IntervalPartition, generate_list_of_lists, check_partition,random_partition, Coarsen_Partition_Interval, random_interval_partition
...
#            phi_Lasso = copy.deepcopy(Lasso(alpha=4 * 0.1*np.sqrt(np.log(d)/ n)).fit(Z, Y).coef_)  # alpha is the regularised constant, if s large, alpha should be large.
 ...
 #           print("Lasso Risk Error = ", error_Lasso)
    ...
    excel_file_path = 'data_greedy_Interval_'+str(d) + '.xlsx'
